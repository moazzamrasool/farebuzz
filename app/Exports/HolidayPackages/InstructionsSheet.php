...
class InstructionsSheet implements FromArray, WithHeadings, WithTitle
{
    public function array(): array
    {
        $rows = [
            ...
            ['Nights / Days', '', 'There is no nights/days column — both are auto-calculated from the number of rows you add for that package on the Itinerary sheet (days = itinerary day count, nights = days - 1), once the whole file finishes importing.'],
            ['', '', ''],
            ...
            ['hotel_category', 'Optional', 'Plain text, e.g. "4 Star Hotels". Only used if hotel_category_override is "yes" — otherwise it is auto-calculated from the star ratings of hotels attached on the Day Hotels sheet, once the whole file finishes importing.'],
            ['hotel_category_override', 'Optional', '"yes" or "no". Defaults to "no" (auto-calculate).'],
            ['meals', 'Optional', 'Plain text, e.g. "Daily Breakfast". Only used if meals_override is "yes" — otherwise it is auto-calculated from the Itinerary sheet\'s meal_tags, once the whole file finishes importing.'],
            ['meals_override', 'Optional', '"yes" or "no". Defaults to "no" (auto-calculate).'],
