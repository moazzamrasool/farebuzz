...
<head>
  ...
  <title>Resend Verification Email – FareBuzzer</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .auth-left-headline { font-size: clamp(28px, 3.5vw, 44px); font-weight: 800; line-height: 1.15; letter-spacing: -1px; margin: 0; }
    ...
    .auth-right { flex: 1; display: flex; align-items: center; justify-content: center; background: #fff; padding: 40px 24px; overflow-y: auto; }
    .auth-form-box { width: 100%; max-width: 400px; }

    .auth-mobile-logo { display: block; font-size: 24px; font-weight: 800; color: #005fcc; text-decoration: none; letter-spacing: -0.5px; margin-bottom: 28px; }
    ...
    .auth-alert { border-radius: 10px; font-size: 13.5px; padding: 12px 14px; margin-bottom: 18px; display: flex; align-items: center; gap: 8px; }
    ...
    .auth-field .field-icon { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: #aaa; font-size: 16px; pointer-events: none; }
    .auth-field input {
...
<div class="auth-wrapper">
    ...
    <div class="auth-left-inner">
      ...
      <div class="auth-left-body">
        <div class="auth-left-tag">
          <i class="bi bi-envelope-check-fill"></i> Account Verification
        </div>
        <h2 class="auth-left-headline">
          Verify Your<br><span>Email Address</span>
        </h2>
        <p class="auth-left-sub">
          Didn't get the link, or has it expired? Request a new verification email below.
        </p>
      </div>
  ...
  <div class="auth-right">
    <div class="auth-form-box">
      ...
      <p class="auth-subtitle">Enter your email and we'll send you a new verification link</p>
      ...
      <form action="{{ route('user.resend-verification') }}" method="POST" novalidate>
        @csrf
      ...
      <div class="auth-footer-link">
        Already verified? <a href="{{ route('user.login') }}">Login</a>
      </div>
